...
from pySC.lattice_properties.response_model import SCgetModelRM_mod
...
def generating_quads_response_matrices(args):
    ...
    dispersion_model = np.concatenate((dx, dy))
    SC.set_magnet_setpoints(quad_index, dk, skewness, order, method)
...
def analyze_ring(SC, twiss, bpm_indices, useIdealRing=True, makeplot=False):
    ...
    rmsx, rmsy = rms_orbits(ring, bpm_indices, SC)
    bx_rms_err, by_rms_err = getBetaBeat(ring, twiss, bpm_indices)
    dx_rms_err, dy_rms_err = getDispersionErr(ring, twiss, bpm_indices)

    print(f"RMS horizontal orbit: {rmsx * 1.e6:.2f} mum, RMS vertical orbit: {rmsy * 1.e6:.2f} mum")
    print(f"RMS horizontal beta beating: {bx_rms_err * 100:.2f}%, RMS vertical beta beating: {by_rms_err * 100:.2f}%")
    print(
        f"RMS relative horizontal dispersion: {dx_rms_err * 1000:.4f} mm, RMS relative vertical dispersion: {dy_rms_err*1000:.4f} mm")
...
def rms_orbits(ring, elements_indexes,SC, trackMode='ORB', Z0=np.zeros(6)):
